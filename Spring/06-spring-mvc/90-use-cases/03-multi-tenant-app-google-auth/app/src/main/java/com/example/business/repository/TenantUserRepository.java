...
public class TenantUserRepository {
    ...
    private static final String GET_ALL_TENANTS_FOR_USER = "SELECT id FROM tenant_system.tenants WHERE id IN " + 
        "(SELECT tenant_id FROM tenant_system.tenant_user WHERE user_id = ?)";
    private static final String GET_ALL_USERS_FOR_TENANT = "SELECT id FROM tenant_system.tenants WHERE id IN " + 
        "(SELECT tenant_id FROM tenant_system.tenant_user WHERE tenant_id = ?)";
    ...
    public void addUserToTenant(String tenantId, String userId, String role) {
        this.jdbcTemplate.update(JOIN_USER_TO_TENANT, tenantId, userId, role);
    }
